...
bool AssetManager::LoadMobsJson() {
    ...
    for (const auto& item : j["mobs"]) {
        ...
        // ── Core ─────────────────────────────────────────────────
        mob.id          = item.value("id",          "");
        mob.displayName = item.value("displayName", "Unknown");
        ...
        if (item.contains("resources")) {
            ...
            mob.resources.modelPath            = r.value("modelPath",            "");
            mob.resources.texturePath          = r.value("texturePath",          "");
