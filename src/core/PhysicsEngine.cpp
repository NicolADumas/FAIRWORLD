...
void PhysicsEngine::StepSimulation(RigidBody& rb, float dt, const fw::ForgeWorld& world) {
    ...
    fw::BlockType centerBlock = world.GetBlock((int)floor(rb.position.x), (int)floor(rb.position.y - rb.radius), (int)floor(rb.position.z));
    rb.isInWater = (centerBlock == fw::BlockType::Water);
