...
bool FairWorldEngine::Init() {
    ...
    // Carica la mesh iniziale del mondo sulla GPU (Chunks) - VECCHIO MONDO DISABILITATO
    /*
    m_world.InitWorld();
...
void FairWorldEngine::Render() {
    if (!m_isVrMode) {
        ...
        // --- SPEEDOMETER HUD (basso destra) ---
        // Mostra la velocità simulata reale in km/h (1 m/s nel motore = 1 m/s reale * 3.6 = km/h)
        if (m_current == GameState::PLAYING) {
            ImVec2 screenSize = ImGui::GetMainViewport()->Size;
            ImDrawList* dlSpeed = ImGui::GetBackgroundDrawList();

            glm::vec3 vel = m_sharedContext->playerVelocity;
            // Velocità orizzontale (quella che il giocatore percepisce muovendosi)
            float hSpeedMs  = sqrtf(vel.x * vel.x + vel.z * vel.z);
            float hSpeedKmh = hSpeedMs * 3.6f;
            // Velocità verticale (caduta / salto)
            float vSpeedKmh = vel.y * 3.6f;

            // Layout: angolo basso destra, sopra la hotbar
            const float PAD   = 16.0f;
            const float BOX_W = 130.0f;
            const float BOX_H = 56.0f;
            ImVec2 boxMin(screenSize.x - BOX_W - PAD, screenSize.y - BOX_H - PAD - 70.0f);
            ImVec2 boxMax(screenSize.x - PAD,          screenSize.y - PAD - 70.0f);

            // Sfondo
            dlSpeed->AddRectFilled(boxMin, boxMax, IM_COL32(8, 8, 12, 160), 5.0f);
            dlSpeed->AddRect(boxMin, boxMax, IM_COL32(60, 60, 70, 180), 5.0f, 0, 1.0f);

            // Numero grande: velocità orizzontale in km/h
            char bigBuf[24];
            snprintf(bigBuf, sizeof(bigBuf), "%.0f", hSpeedKmh);

            // Calcoliamo la larghezza per centrare il numero + unità
            ImVec2 numSize  = ImGui::CalcTextSize(bigBuf);
            ImVec2 unitSize = ImGui::CalcTextSize(" km/h");
            float  totalW   = numSize.x + unitSize.x;
            float  startX   = boxMin.x + (BOX_W - totalW) * 0.5f;
            float  numY     = boxMin.y + 8.0f;

            dlSpeed->AddText(ImGui::GetFont(), 22.0f, ImVec2(startX, numY),
                             IM_COL32(240, 240, 240, 240), bigBuf);
            dlSpeed->AddText(ImGui::GetFont(), 11.0f, ImVec2(startX + numSize.x + 2.0f, numY + 9.0f),
                             IM_COL32(150, 150, 160, 200), "km/h");

            // Riga secondaria: componente verticale (per salto/caduta)
            char vBuf[32];
            snprintf(vBuf, sizeof(vBuf), "v %+.0f km/h", vSpeedKmh);
            ImVec2 vSize = ImGui::CalcTextSize(vBuf);
            ImU32 vCol = (vel.y < -5.0f) ? IM_COL32(255, 100, 80, 220)   // caduta veloce
                       : (vel.y >  2.0f) ? IM_COL32(100, 200, 255, 200)  // salto
                       :                   IM_COL32(110, 110, 120, 170);  // neutro
            dlSpeed->AddText(ImVec2(boxMin.x + (BOX_W - vSize.x) * 0.5f, boxMin.y + 36.0f), vCol, vBuf);
        }
