...
BlockRegistry::BlockRegistry() {
    ...
    m_fallbackBlock.baseColor = glm::vec3(1.0f, 0.0f, 1.0f); // Magenta error block
}
...
void BlockRegistry::RegisterDefaultBlocks() {
    ...
    m_blocks[1].baseColor = glm::vec3(0.2f, 0.8f, 0.2f);
    m_blocks[1].roughness = 0.9f;
    ...
    m_blocks[2].baseColor = glm::vec3(0.5f, 0.3f, 0.1f);
    m_blocks[2].roughness = 1.0f;
    ...
    m_blocks[3].baseColor = glm::vec3(0.5f, 0.5f, 0.5f);
    m_blocks[3].roughness = 0.7f;
...
bool BlockRegistry::LoadFromJson(const std::string& filepath) {
    ...
    if (j.contains("blocks") && j["blocks"].is_array()) {
        for (const auto& jBlock : j["blocks"]) {
            ...
            BlockDefinition& def = m_blocks[id];
            def.id = id;
            def.stringId = jBlock.value("stringId", "fairworld:unknown");
            def.displayName = jBlock.value("displayName", "Unknown");
            
            if (jBlock.contains("baseColor")) {
                auto color = jBlock["baseColor"];
                def.baseColor = glm::vec3(color[0], color[1], color[2]);
            }
            
            def.metallic = jBlock.value("metallic", 0.0f);
            def.roughness = jBlock.value("roughness", 1.0f);
            def.emissiveStrength = jBlock.value("emissiveStrength", 0.0f);
            
            if (jBlock.contains("emissiveColor")) {
                auto color = jBlock["emissiveColor"];
                def.emissiveColor = glm::vec3(color[0], color[1], color[2]);
            }
            
            def.ior = jBlock.value("ior", 1.45f);
            def.clearcoat = jBlock.value("clearcoat", 0.0f);
            ...
            def.lightEmissionLevel = jBlock.value("lightEmissionLevel", 0);
...
bool BlockRegistry::SaveToJson(const std::string& filepath) {
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        jBlock["baseColor"] = {def.baseColor.x, def.baseColor.y, def.baseColor.z};
        jBlock["metallic"] = def.metallic;
        jBlock["roughness"] = def.roughness;
        jBlock["emissiveStrength"] = def.emissiveStrength;
        jBlock["emissiveColor"] = {def.emissiveColor.x, def.emissiveColor.y, def.emissiveColor.z};
        jBlock["ior"] = def.ior;
        jBlock["clearcoat"] = def.clearcoat;
...
void BlockRegistry::UpdateBlock(uint8_t id, const BlockDefinition& def) {
    if (id > 0 && id < 256) {
        m_blocks[id] = def;
        m_stringToIdMap[def.stringId] = id;
    }
...
const BlockDefinition& BlockRegistry::GetBlock(const std::string& stringId) const {
...
BlockDefinition& BlockRegistry::GetBlockMutable(uint8_t id) {
    if (id < 256) return m_blocks[id];
    return m_fallbackBlock;
}
