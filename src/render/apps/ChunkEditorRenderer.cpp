...
struct ChunkEditorPushConstants {
    glm::mat4 mvp;
...
bool ChunkEditorRenderer::Initialize(VkDevice device, VkRenderPass renderPass, VkExtent2D extent) {
    m_extent = extent;
    return true;
}

void ChunkEditorRenderer::Draw(VkCommandBuffer cmd, SharedContext* context, glm::mat4 viewMatrix, glm::mat4 projMatrix) {
    ...
    // Rispetta il layout madre UI con split al 45% a sinistra
    VkViewport viewport{};
    ...
    ChunkEditorFrustum frustum;
    frustum.extract(viewProjMatrix);

    ChunkEditorPushConstants pcData{};
    ...
    for (auto entity : view) {
        ...
        vkCmdPushConstants(cmd, m_pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(ChunkEditorPushConstants), &pcData);
...
void ChunkEditorRenderer::Cleanup(VkDevice device) {
    // Risorse gestite dal RenderManager madre (Regina)
}
