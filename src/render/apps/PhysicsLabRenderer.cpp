...
bool PhysicsLabRenderer::Initialize(VkDevice device, VkRenderPass renderPass, VkExtent2D extent) {
    m_extent = extent;
    return true;
}

void PhysicsLabRenderer::Draw(VkCommandBuffer cmd, SharedContext* context, glm::mat4 viewMatrix, glm::mat4 projMatrix) {
    ...
    for (auto entity : view) {
        ...
        vkCmdPushConstants(cmd, m_pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PhysicsLabPushConstantData), &pcData);
...
void PhysicsLabRenderer::Cleanup(VkDevice device) {
}
