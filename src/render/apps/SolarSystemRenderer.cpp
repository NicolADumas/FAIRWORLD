...
struct SolarSystemPushConstants {
    glm::mat4 mvp;
...
bool SolarSystemRenderer::Initialize(VkDevice device, VkRenderPass renderPass, VkExtent2D extent) {
    m_extent = extent;
    return true;
}
...
void SolarSystemRenderer::Draw(VkCommandBuffer cmd, SharedContext* context, glm::mat4 viewMatrix, glm::mat4 projMatrix) {
    ...
    // Nel Sistema Solare si renderizza a tutto schermo o col viewport split a seconda della modalità
    VkViewport viewport{};
    ...
    SolarSystemFrustum frustum;
    frustum.extract(viewProjMatrix);

    SolarSystemPushConstants pcData{};
...
void SolarSystemRenderer::Cleanup(VkDevice device) {
    // Risorse gestite dal RenderManager regina
}
